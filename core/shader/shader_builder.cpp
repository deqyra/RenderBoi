...
const ShaderProgram ShaderBuilder::_minimalShaderProgram = ShaderBuilder::buildMinimalShaderProgram();
const unsigned int ShaderBuilder::shadingLanguageVersion = 420;
const std::string ShaderBuilder::shadingLanguageProfile = "core";

ShaderProgram ShaderBuilder::buildMinimalShaderProgram()
{
    static bool runOnce = false;
    if (runOnce) return _minimalShaderProgram;

    runOnce = true;
    return buildShaderProgramFromConfig(ShaderConfig::MinimalConfig);
}
...
Shader ShaderBuilder::buildShaderStageFromConfig(ShaderInfo::ShaderStage stage, ShaderConfig config)
{
    ...
    static const std::unordered_map<ShaderInfo::ShaderStage, std::string>& stageTemplatePaths = getStageTemplatePaths();
...
Shader ShaderBuilder::buildShaderStageFromFile(ShaderInfo::ShaderStage stage, std::string filename, std::vector<ShaderInfo::ShaderFeature> supportedFeatures)
{
    ...
    if (!supportedFeatures.size())
    {
        ...
        if (it != ShaderInfo::SupportedFeatures.end())
...
{
    std::vector<std::string> includeFilenames = findIncludeFilenamesInSource(text);
    
    for (auto it = includeFilenames.begin(); it != includeFilenames.end(); it++)
    {
...
std::vector<std::string> ShaderBuilder::findIncludeFilenamesInSource(std::string text)
{
    ...
    while (offset != std::string::npos)
    {
        ...
        std::string filename = includeArgument.substr(0, includeArgument.size() - 1);

        // If any delimiting chars are left in the string after stripping, that makes the argument illegal
        if (filename.find(secondDelimiter) != std::string::npos)
        {
...
void ShaderBuilder::makeNamedString(std::string name, std::string sourceFilename)
{
	...
	if (!file.is_open())
    {
		std::string s = "Shader source file \"" + sourceFilename + "\" could not be opened.";
        throw std::runtime_error(s.c_str());
    }
...
const std::string& ShaderBuilder::generateExtensionDirectives()
{
    ...
    if (!runOnce)
    {
        ...
        const std::unordered_map<std::string, std::string>& extensions = getShadingLanguageExtensions(); 
...
const std::unordered_map<std::string, std::string>& ShaderBuilder::getShadingLanguageExtensions()
{
    static bool runOnce = false;
    static std::unordered_map<std::string, std::string> extensions;
    if (!runOnce)
    {
        extensions["GL_ARB_shading_language_include"] = "require";
...
const std::unordered_map<ShaderInfo::ShaderStage, std::string>& ShaderBuilder::getStageTemplatePaths()
{
    static bool runOnce = false;
    ...
    if (!runOnce)
    {
        map[ShaderInfo::ShaderStage::Vertex]    = "assets/templates/vertex_template.vert";
        map[ShaderInfo::ShaderStage::Geometry]  = "assets/templates/geometry_template.geom";
        map[ShaderInfo::ShaderStage::Fragment]  = "assets/templates/fragment_template.frag";
...
const std::unordered_map<ShaderInfo::ShaderFeature, std::string>& ShaderBuilder::getFeatureDefineMacros()
{
    static bool runOnce = false;
...
{
    static const std::unordered_map<ShaderInfo::ShaderFeature, std::string>& defineMacros = getFeatureDefineMacros();
