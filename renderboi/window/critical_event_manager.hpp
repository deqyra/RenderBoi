#ifndef RENDERBOI__WINDOW__CRITICAL_EVENT_MANAGER_HPP
#define RENDERBOI__WINDOW__CRITICAL_EVENT_MANAGER_HPP
...
public:
    ...
    virtual void processCriticalEvents();
    ...
    virtual void detachCriticalProcess(unsigned int registrationId) override;
    ...
    virtual void queueCriticalEvent(GLWindowCriticalEvent event) override;

private:
    /// @brief Process a single critical event in the queue.
    ///
    /// @param event Literal describing the event to process.
    void _processCriticalEvent(GLWindowCriticalEvent event);
...
#endif//RENDERBOI__WINDOW__CRITICAL_EVENT_MANAGER_HPP