...
ShadowSandbox::ShadowSandbox(
    GLWindow& window, const GLSandboxParameters& params
)
...
void ShadowSandbox::run() {
    ...
    ShaderProgram fullLightShader =
        ShaderBuilder::BuildShaderProgramFromConfig(lightConfig);
    ...
    // ShaderBuilder::BuildShaderProgramFromConfig(depthConfig);
    ...
    Material floorMaterial = Material(
        num::Vec3(0.f), num::Vec3(1.f),
        num::Vec3(0.3203125f, 0.254296875f, 0.180859375f), 128.f
    );
    Texture2D floorTex = Texture2D("wood.png", PixelSpace::sRGB);
    floorMaterial.pushDiffuseMap(floorTex);
    Object* fp = nullptr;

    Object& floorObj = Factory::MakeObjectWithMesh<MeshType::Plane>(
        scene, "Floor", planeParameters, floorMaterial, blinnPhongShader
    );

    // WALLS
    Material wallMaterial = Material(
        num::Vec3(0.f), num::Vec3(1.f), num::Vec3(0.25f, 0.15f, 0.15f), 2.f
    );
    Texture2D wallTex = Texture2D("wall.jpg", PixelSpace::sRGB);
    wallMaterial.pushDiffuseMap(wallTex);

    // XY wall
    Object& xyWallObj = Factory::MakeObjectWithMesh<MeshType::Plane>(
        scene, "XY wall", planeParameters, wallMaterial, blinnPhongShader
    );

    // YZ wall
    Object& yzWallObj = Factory::MakeObjectWithMesh<MeshType::Plane>(
        scene, "YZ wall", planeParameters, wallMaterial, blinnPhongShader
    );
    ...
    // TORUS
    Object& torusObj = Factory::MakeObjectWithMesh<MeshType::Torus>(
        scene, "Torus", torusParameters, Materials::Default, blinnPhongShader
    );

    // LIGHT
    Object& lightObj = Factory::MakeObjectWithMesh<MeshType::Cube>(
        scene, "Light cube",
        {
            0.3f, {0.f, 0.f, 0.f},
             false
    },
        Materials::Default, fullLightShader
    );
    PointLight light(LightBaseRange);
    lightObj.componentMap().addComponent<ComponentType::Light>(light);

    // CAMERA
    Object& cameraObj = scene.newObject("Camera");
    ...
    const num::Vec3 X = WorldTransform::X;
    const num::Vec3 Y = WorldTransform::Y;
    const num::Vec3 Z = WorldTransform::Z;
    ...
    torusObj.transform().translateBy<Ref::World>(
        { WallSize / 2.f, WallSize / 2.f, WallSize / 2.f }
    );
    ...
    ControlledEntityManager<KeyboardMovementScript>
        keyboardMovementScriptManager(cameraAsBasisProvider);
    cameraObj.componentMap().addComponent<ComponentType::Script>(
        static_cast<Script&>(keyboardMovementScriptManager.entity())
    );

    // Add script component to camera: GamepadMovementScript
    GamepadMovementScript gamepadMovementScript(cameraAsBasisProvider);
    cameraObj.componentMap().addComponent<ComponentType::Script>(
        static_cast<Script&>(gamepadMovementScript)
    );
    ...
    splitter.registerInputProcessor(
        static_cast<InputProcessor&>(cameraAspectRatioManager)
    );
    splitter.registerInputProcessor(static_cast<InputProcessor&>(sandboxScript)
    );
    splitter.registerInputProcessor(static_cast<InputProcessor&>(
        keyboardMovementScriptManager.eventTranslator()
    ));
    splitter.registerInputProcessor(
        static_cast<InputProcessor&>(windowManager.entity())
    );
    splitter.registerInputProcessor(
        static_cast<InputProcessor&>(windowManager.eventTranslator())
    );

    // Register all gamepad input processors to the splitter
    splitter.registerGamepadInputProcessor(
        static_cast<GamepadInputProcessor&>(gamepadMovementScript)
    );
    splitter.registerGamepadInputProcessor(
        static_cast<GamepadInputProcessor&>(gamepadCameraManager)
    );

    // Register the logger as both a classic input processor and a gamepad input
    // processor
    splitter.registerInputProcessor(static_cast<InputProcessor&>(logger));
    splitter.registerGamepadInputProcessor(
        static_cast<GamepadInputProcessor&>(logger)
    );

    // Register the splitter to the window and the gamepad
    _window.registerInputProcessor(static_cast<InputProcessor&>(splitter));
    if (_gamepad) {
        _gamepad->registerInputProcessor(
            static_cast<GamepadInputProcessor&>(splitter)
        );
    }
...
ShadowSandboxScript::ShadowSandboxScript(
    Object& lightObj, Object& torusObj
)
    : _lightObj(lightObj)
    , _torusObj(torusObj)
...
void ShadowSandboxScript::update(float timeElapsed) {
    ...
    if (!_pause) {
        num::Vec3 newLightPos =
            _lightStartingPos +
            LightMovementAxis * _sine.get() * (LightMovementAmplitude / 2.f);
        _lightObj.transform().setPosition<Ref::World>(newLightPos);

        float delta = _speedFactor * timeElapsed;
        _torusObj.transform().rotateBy<Ref::World>(
            glm::radians(45.f * delta), TorusRotationAxis
        );
    }
}

void ShadowSandboxScript::processKeyboard(
    GLWindow&                   _window,
